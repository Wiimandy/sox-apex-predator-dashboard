...
def build_interactive_script(market_rows: list[dict], client_config: dict, tickers: list[str], source_label: str) -> str:
  ...
  <script>
    ...
    const copyEditorState = {
      ...
      editing: false,
      edits: {},
      storageKey: `apex-sox-copy-edits:${document.body.dataset.page || 'page'}`,
    };
    ...
    function getCopyEditableElements() {
      const selectors = [
        '.report-hero h1',
        '.hero-copy p',
        '.hero-stat span',
        ...
        '.rule-help',
        '.rule-table th',
        '.rule-table td:not(.amount-cell)',
        '.parameter-analysis h2',
        '.parameter-help',
        '.strategy-details .eyebrow',
        '.strategy-details h2',
        '.strategy-details p',
        '.detailed-performance .comparison-head span',
        '.detailed-performance .comparison-head strong',
        '.detailed-performance .comparison-row span:first-child',
        '.comparison-footnote',
        '.nav-button',
        '.note',
      ];
      ...
      return [...new Set(nodes)].filter(element => (
        cleanEditorText(element.innerText || element.textContent)
        && !element.closest('[data-editor-toolbar]')
        && !element.closest('#chart')
        && !['INPUT', 'BUTTON', 'SCRIPT', 'STYLE'].includes(element.tagName)
      ));
    }

    function getCopyEditorContext(element) {
      const section = element.closest('section, header');
      const heading = section?.querySelector('h1, h2');
      const sectionName = heading && heading !== element ? cleanEditorText(heading.innerText) : '';
      return [section?.className || section?.tagName?.toLowerCase() || 'body', sectionName].filter(Boolean).join(' / ');
    }

    function loadCopyEdits() {
      try {
        copyEditorState.edits = JSON.parse(localStorage.getItem(copyEditorState.storageKey) || '{}') || {};
      } catch (error) {
        copyEditorState.edits = {};
      }
    }

    function saveCopyEdits() {
      localStorage.setItem(copyEditorState.storageKey, JSON.stringify(copyEditorState.edits));
      updateCopyEditorStatus();
    }

    function updateCopyEditorStatus() {
      const status = document.getElementById('copyEditorStatus');
      if (!status) return;
      const changedCount = Object.values(copyEditorState.edits).filter(edit => edit && edit.changed).length;
      status.textContent = `${copyEditorState.editing ? '編輯中' : '編輯關閉'}｜已修改 ${changedCount} 處`;
    }

    function refreshCopyEditableText() {
      const elements = getCopyEditableElements();
      elements.forEach((element, index) => {
        const key = `copy-${String(index + 1).padStart(3, '0')}`;
        const originalText = element.dataset.copyOriginal || cleanEditorText(element.innerText || element.textContent);
        element.dataset.copyKey = key;
        element.dataset.copyOriginal = originalText;
        element.dataset.copyContext = getCopyEditorContext(element);
        const saved = copyEditorState.edits[key];
        if (saved && saved.text && document.activeElement !== element) {
          element.innerText = saved.text;
        }
        element.dataset.copyEditable = 'true';
        element.contentEditable = copyEditorState.editing ? 'true' : 'false';
        element.spellcheck = false;
        if (!element.dataset.copyEditorBound) {
          element.dataset.copyEditorBound = 'true';
          element.addEventListener('input', () => {
            const text = cleanEditorText(element.innerText);
            const original = element.dataset.copyOriginal || '';
            copyEditorState.edits[element.dataset.copyKey] = {
              key: element.dataset.copyKey,
              tag: element.tagName.toLowerCase(),
              context: element.dataset.copyContext || '',
      ...
      document.body.classList.toggle('copy-editing', copyEditorState.editing);
      updateCopyEditorStatus();
    }

    function setCopyEditing(enabled) {
      copyEditorState.editing = enabled;
      const toggle = document.getElementById('toggleCopyEditor');
      if (toggle) toggle.textContent = enabled ? '關閉編輯' : '開啟編輯';
      refreshCopyEditableText();
    }

    function exportCopyJson() {
      refreshCopyEditableText();
      const entries = getCopyEditableElements().map(element => {
        const originalText = element.dataset.copyOriginal || '';
        const text = cleanEditorText(element.innerText);
        return {
          key: element.dataset.copyKey,
          tag: element.tagName.toLowerCase(),
      ...
      const payload = {
        schemaVersion: 1,
        source: 'apex-sox-copy-editor',
        pageType: document.body.dataset.page || '',
        path: window.location.pathname,
        exportedAt: new Date().toISOString(),
        changedEdits: entries.filter(entry => entry.changed),
        allEditableText: entries,
      };
      ...
      link.href = url;
      link.download = `apex-sox-copy-${payload.pageType || 'page'}-${stamp}.json`;
    ...
    function clearCopyEdits() {
      copyEditorState.edits = {};
      localStorage.removeItem(copyEditorState.storageKey);
      getCopyEditableElements().forEach(element => {
        if (element.dataset.copyOriginal) element.innerText = element.dataset.copyOriginal;
      });
      refreshCopyEditableText();
    }

    function setupCopyEditor() {
      if (copyEditorState.initialized) {
        refreshCopyEditableText();
        return;
      }
      const toolbar = document.querySelector('[data-editor-toolbar]');
      if (!toolbar) return;
      copyEditorState.initialized = true;
      loadCopyEdits();
      document.getElementById('toggleCopyEditor')?.addEventListener('click', () => setCopyEditing(!copyEditorState.editing));
      document.getElementById('exportCopyJson')?.addEventListener('click', exportCopyJson);
      document.getElementById('clearCopyEdits')?.addEventListener('click', clearCopyEdits);
      document.addEventListener('click', event => {
        if (copyEditorState.editing && event.target.closest('a[data-copy-editable]')) {
          event.preventDefault();
        }
      }, true);
      refreshCopyEditableText();
    ...
    function applyDates(pushState = true) {
      ...
      try {
        ...
        status.textContent = '';
        if (copyEditorState.initialized) refreshCopyEditableText();
      } catch (error) {
...
""" if is_detail else ""
    ...
    if is_detail:
        ...
        thesis_body = "白話文：指數跌愈多愈買。一旦跌到市場非常恐慌，就會進行狙擊加碼。另外，每個月的加碼份數也會因為統計規律會有所變動。"
        vix_factor_title = "恐慌程度 VIX"
        rmdd_factor_body = "跌破回撤防線時，啟動「標準加碼」"
        vix_factor_body = "市場進入恐慌區時，搭配回撤訊號，觸發「狙擊買入」"
        seasonality_factor_body = "不同月份加碼份數會有所不同"
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .copy-editor-toolbar {{ display: flex; align-items: center; justify-content: space-between; gap: 14px; padding: 10px 22px; background: rgba(5, 13, 22, 0.96); border-bottom: 1px solid var(--line); color: var(--soft); font-size: 12px; }}
    .copy-editor-toolbar b {{ color: var(--text); font-size: 13px; }}
    .copy-editor-toolbar span {{ color: var(--muted); }}
    .copy-editor-actions {{ display: flex; align-items: center; gap: 8px; }}
    .copy-editor-toolbar button {{ min-height: 32px; border: 1px solid var(--line); border-radius: 6px; padding: 0 11px; background: rgba(13, 26, 39, 0.92); color: var(--soft); font: inherit; font-weight: 700; cursor: pointer; }}
    .copy-editor-toolbar button.primary {{ border-color: rgba(98, 230, 255, 0.58); background: var(--cyan); color: #031018; }}
    .copy-editor-toolbar button:hover {{ border-color: rgba(98, 230, 255, 0.62); }}
    [data-copy-editable="true"] {{ transition: outline-color 160ms ease, background 160ms ease; }}
    body.copy-editing [data-copy-editable="true"] {{ outline: 1px dashed rgba(98, 230, 255, 0.26); outline-offset: 4px; cursor: text; }}
    ...
    @media (max-width: 900px) {{
      ...
      .copy-editor-toolbar {{ align-items: stretch; flex-direction: column; padding: 10px 12px; }}
      .copy-editor-actions {{ flex-wrap: wrap; }}
...
<body data-page="{page_type}">
  <div class="copy-editor-toolbar" data-editor-toolbar>
    <div><b>文字編輯工具</b> <span id="copyEditorStatus">編輯關閉｜已修改 0 處</span></div>
    <div class="copy-editor-actions">
      <button id="toggleCopyEditor" class="primary" type="button">開啟編輯</button>
      <button id="exportCopyJson" type="button">輸出 JSON</button>
